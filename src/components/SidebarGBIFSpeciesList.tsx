...
import { fetchGBIFSpeciesCounts } from '@/services/gbif';

interface SidebarGBIFSpeciesListProps {
...
export default function SidebarGBIFSpeciesList({
    coordinates,
    radius,
    filterTerm,
    selectedStatuses,
    ...
    useEffect(() => {
        async function loadSpecies() {
            ...
            try {
                const results = await fetchGBIFSpeciesCounts(coordinates, radius);
                setSpecies(results);
            } catch (err) {
                setError('Failed to load GBIF species list');
                console.error(err);
            } finally {
                setLoading(false);
            }
